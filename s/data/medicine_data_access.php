...
<?php
	...
	{
		$sql = "INSERT INTO medicine (id,med_name,indication,generic,price,quantity) VALUES (ID.nextval,'$medicine[medName]','$medicine[indication]','$medicine[generic]','$medicine[price]','$medicine[quantity]')";
		 $result = executeSQL($sql);
        return $result;
	}

     function addOrderToDb($order){
	$sql = "INSERT INTO order_report (orderId, orderName, orderQuantity, orderCost) VALUES (ID.nextval, '$order[medName]', '$order[quantity]', '$order[total_q]')";
	$result = executeSQL($sql);
	return $result;
    }

	function donateBLoodToDb($blood){
        $sql = "INSERT INTO donate_blood (donor_id, donar_name, donar_BG, donar_contact, donar_area) VALUES (ID.nextval, '$blood[name]', '$blood[bg]', '$blood[contact]', '$blood[area]')";
        $result = executeSQL($sql);
        return $result;
   	 }
	...
	function getAllDonorsFromDb(){
     ...
     function getMedicineByIdFromDb($medicineId){
        ...
        $medicine = oci_fetch_array($result);
